...
def displayJpeg(strip, imName, wait_ms=500):
	...
	for y in range(0, size[1]):
		for x in range(0, size[0]):
			if y%2 != 0:
				strip.setPixelColor(i, Color(buf[31-x,y][0], buf[31-x,y][1], buf[31-x,y][2]))
			else:
...
if __name__ == '__main__':
	...
	while True:
		...
		displayGIF(strip, "gifs/breadboarder_hue.gif")

		print ('Color wheel')
		for i in range(0,3):
			displayGIF(strip, "gifs/colorwheel.gif")

		displayGIF(strip, "gifs/pong.gif")
